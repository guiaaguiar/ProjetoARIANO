...
| Ação | Pacote | Motivo |
|------|--------|--------|
| 🗑️ **REMOVER** (frontend) | `d3`, `@types/d3` | Substituído por NetworkX (backend) + react-force-graph |
| 🗑️ **REMOVER** (frontend) | `sigma`, `@sigma/edge-curve` | Substituído por react-force-graph |
...
| ✅ **ADICIONAR** (backend) | `networkx>=3.3` | Layout (`spring_layout`), comunidades (`louvain_communities`), temas semânticos |
| ✅ **ADICIONAR** (frontend) | `react-force-graph` | Motor de visualização interativa premium |
| ✅ **MANTER** (frontend) | `framer-motion` | Animações de transição e UI |
...
| `/user/ecossistema` | **EcossistemaPage** | Grafo pessoal (layout NetworkX pre-computado, renderizado via react-force-graph) — mostra APENAS nós conectados ao usuário logado. |
...
| `/admin/grafo` | GrafoPage | Grafo completo (NetworkX + react-force-graph) |
...
| `frontend/src/components/MiniGraph.tsx` | **CRIAR** | Mini-grafo interativo do nó criado + conexões (posições pre-computadas via NetworkX) |
...
| 4.4.3 | **Linha do Tempo de Diálogo (Agentes)** | Interface de mensagens flutuantes e imersivas entre Agentes de IA. Ao contrário de um console técnico, os agentes "conversam" entre si e com o grafo, explicando o raciocínio em linguagem clara. Cada balão de diálogo reflete um passo do Graph-CoT, humanizando a percepção da IA. |
| 4.4.4 | **Grafo Vivo (Hero Animation)** | O Grafo é o elemento central (Hero) da tela de cadastro. Ele se expande e reage aos sinais enviados pelos agentes. Enquanto a IA processa, o grafo anima mostrando: (1) o nó do usuário surgindo, (2) arestas de skills se conectando, (3) editais compatíveis piscando e ligando-se ao usuário com glows neon. |
...
| `frontend/src/pages/user/EcossistemaPage.tsx` | **CRIAR** | Grafo pessoal interativo (layout NetworkX) |
