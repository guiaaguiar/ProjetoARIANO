...
import { Menu, Home, CheckCircle, Network, LayoutTemplate, LogOut } from "lucide-react";
...
const navItems = [
  { icon: Home, label: "Início", path: "/user" },
  { icon: CheckCircle, label: "Matches Recomendados", path: "/user/matches" },
  { icon: Network, label: "Meu Ecossistema", path: "/user/ecossistema" },
  { icon: LayoutTemplate, label: "Comunidades", path: "/user/comunidades" },
];

export function UserSidebarContent({ collapsed = false, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="p-0 w-52 bg-sidebar border-r border-border">
              <div className="flex flex-col h-full">
                <UserSidebarContent onNavigate={() => setOpen(false)} />
              </div>
            </SheetContent>
          </Sheet>
